...
def rec(typ,fid,ss,flags=0):
 b=body(ss);return struct.pack('<4sIIIIHH',typ.encode(),len(b),flags,fid,0,131,0)+b
def group(label,typ,data):return struct.pack('<4sI4siIHH',b'GRUP',24+len(data),label,typ,0,0,0)+data
ids={0xa0f2f,0xa0f30,0xa0f31,0xa0f33,0x21e76,0x1d568,0x1d162,0x18aa2,0xbeba4,0xbeba5,0x1ab276}
...
for i,(s,b) in enumerate(race):
 if s=='DATA':
  b=bytearray(b);struct.pack_into('<I',b,32,struct.unpack_from('<I',b,32)[0]|0x200000);race[i]=(s,bytes(b))
add('RACE',RACE,race)
armor=clone(0xa0f31,{'EDID':S('DoroSkin'),'RNAM':U(RACE),'DESC':S('')},['INDX','MODL'])
ix=next(i for i,(s,b) in enumerate(armor) if s=='DATA');armor[ix:ix]=[('INDX',b'\0\0'),('MODL',U(AA))]
...
def package(base,fid,name,state=None,radius=None):
 ...
 for i,(s,b) in enumerate(ss):
  if s=='PKDT':
   b=bytearray(b);flags=struct.unpack_from('<I',b)[0]&~(1<<20)
   if state in [1,2]:flags|=1<<4
   struct.pack_into('<I',b,0,flags);ss[i]=(s,bytes(b))
 if state is not None:
  i=next(i for i,(s,b) in enumerate(ss) if s=='PSDT')+1;ss.insert(i,('CTDA',condition(state)))
 if radius is not None:
  # Sandbox current-location radius is its first Location input.
  i=next(i for i,(s,b) in enumerate(ss) if s=='PLDT');b=bytearray(ss[i][1]);struct.pack_into('<I',b,8,radius);ss[i]=('PLDT',bytes(b))
 add('PACK',fid,ss)
...
# Dedicated sound descriptors and audio template leave vanilla bears unchanged.
sound=clone(0xbeba5,{'EDID':S('DoroVoice'),'LNAM':b'\0'*4,'BNAM':struct.pack('<bbBBH',0,2,128,0,300)},['ANAM'])
i=next(i for i,(s,b) in enumerate(sound) if s=='ONAM');sound[i:i]=[('ANAM',S('Sound\\FX\\Doro\\Doro01.wav')),('ANAM',S('Sound\\FX\\Doro\\Doro02.wav'))]
...
script=S('DoroCompanionScript')[:-1];vmad=struct.pack('<HHHH',6,2,1,len(script))+script+b'\0'+struct.pack('<H',0)
...
for i,(s,b) in enumerate(npc):
 if s=='ACBS':
  b=bytearray(b);struct.pack_into('<I',b,0,2|32|64|4096);struct.pack_into('<H',b,6,20);struct.pack_into('<H',b,14,0);npc[i]=(s,bytes(b));npc.insert(i+1,('SNAM',U(0x1c21c)+b'\0'));break
for i,(s,b) in enumerate(npc):
 if s=='AIDT':
  b=bytearray(b);b[0]=1;b[1]=4;b[3]=0;b[5]=2;b[6]=0;npc[i]=(s,bytes(b))
i=next(i for i,(s,b) in enumerate(npc) if s=='AIDT')+1;npc[i:i]=[('PKID',U(x)) for x in [FOLLOW,WAIT,HOME]]
add('NPC_',NPC,npc)
...
pos=bytearray(next(b for s,b in van[0x1d162][2] if s=='DATA'));x,y,z,rx,ry,rz=struct.unpack('<6f',pos)
# Offset from Dogmeat's original editor position, within the Red Rocket forecourt.
struct.pack_into('<6f',pos,0,x+160,y+100,z,rx,ry,rz)
dog=json.loads((ROOT/'build/dogmeat_bounds.json').read_text());scale=(dog['max'][2]-dog['min'][2])/155.31235
placed=rec('ACHR',PLACED,[('EDID',S('DoroRef')),('NAME',U(NPC)),('XSCL',F(scale)),('DATA',bytes(pos))],0x400)
# Use the existing Commonwealth persistent cell; no worldspace settings are changed.
...
header=rec('TES4',0,[('HEDR',struct.pack('<fII',1.0,count,0x900)),('CNAM',S('Doro FO4 conversion')),('SNAM',S('Standalone YaoGuai-based Doro follower prototype')),('MAST',S('Fallout4.esm')),('DATA',b'\0'*8)])
...
(ROOT/'build/plugin_manifest.json').write_text(json.dumps({'plugin':'DoroFollower.esp','npc_local_form':'00000803','reference_local_form':'0000080B','scale':scale,'placement':[x+160,y+100,z],'new_records':count-1,'commands':['Follow','Wait','Inventory','Return to Red Rocket'],'runtime_tested':False},indent=2))
print('PLUGIN_WRITTEN',len(plugin),'bytes','scale',scale)
# Re-read every record with the independent record parser and verify VMAD/form counts.
parsed=list(records(OUT/'DoroFollower.esp'));assert len(parsed)==count+1
assert sum(t=='ACHR' for t,*_ in parsed)==1
assert any(t=='NPC_' and f==NPC and edid(b)=='DoroCompanion' for t,f,fl,b in parsed)
print('PLUGIN_STRUCTURAL_CHECK_PASSED',len(parsed),'records')

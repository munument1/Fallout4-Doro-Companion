...
# Preserve original bone hierarchy and rest transforms for a first skinning pass.
source=bpy.data.objects['skeleton.nif:ARMATURE']
rig=source.copy();rig.data=source.data.copy();col.objects.link(rig);rig.name='Doro_YaoGuai_Rig';rig.hide_set(False);rig.hide_render=True;rig.show_in_front=True
def bind(o,weights):
    if o.type=='CURVE':
    ...
    for b,w in weights.items():
        if b in rig.data.bones:o.vertex_groups.new(name=b).add(list(range(len(o.data.vertices))),w,'REPLACE')
    m=o.modifiers.new('YaoGuai skin','ARMATURE');m.object=rig
    return o
for o in list(col.objects):
    if o.type not in {'MESH','CURVE'}:continue
    if o==body:continue
    if o.name.startswith(('Mouth','Tongue')):bind(o,{'HEAD':.94,'Jaw':.06})
    elif o.name=='Tiny tail':bind(o,{'Pelvis':1})
    else:bind(o,{'HEAD':1})
groups={n:body.vertex_groups.new(name=n) for n in ['Pelvis','SPINE1','SPINE2']+[s+'Leg_'+end+'_'+joint for s in ['L','R'] for end in ['Front','Rear'] for joint in ['Thigh','Knee','Ankle']]}
for v in body.data.vertices:
    x,y,z=body.matrix_world@v.co
    if z<51 and abs(x)>13 and (y> -16 or y< -53):
        side='L' if x<0 else 'R';end='Front' if y>-16 else 'Rear';prefix=side+'Leg_'+end+'_'
        if z<25:t=max(0,min(1,z/25));weights={prefix+'Ankle':1-t,prefix+'Knee':t}
        else:t=max(0,min(1,(z-25)/26));weights={prefix+'Knee':1-t,prefix+'Thigh':t}
    else:
        t=max(0,min(1,(y+65)/85));weights={'Pelvis':1-t,'SPINE2':t}
    for n,w in weights.items():
        if w:groups[n].add([v.index],w,'REPLACE')
m=body.modifiers.new('YaoGuai skin','ARMATURE');m.object=rig
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'Doro_YaoGuai_v01.blend'))
...
cam.location=(230,310,185);aim(cam,(0,-17,69));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'Doro_YaoGuai_v01.blend'))
